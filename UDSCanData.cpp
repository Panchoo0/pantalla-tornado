...
void UDSCanData::run() {
    ...
    // Lista de dids que serán consultados
    int dids[] = {
        0xA011, 0xA012, 0xA013, 0xA016, 0xA018, 0xA019, 0xA01B, 0xA01E, 0xA01F, 0xA020,
    ...
    while (true) {
        ...
        for (int didIndex = 0; didIndex < didsLen; didIndex++) {
            ...
            while (true) {
                ...
                if (nbytes < 0)
                {
                    qInfo() << "(UDS) Error sending socket error " << errno << " which means " << strerror(errno);
                    emit dbgMessage("(UDS) Error al enviar datos");
                    continue;
                } else {
                    // qInfo() << "(UDS) Mensaje correctamente enviado";
                }
                ...
                if (res < 0) {
                    qInfo() << "(UDS) Error al leer datos" << errno << strerror(errno);
                } else {
